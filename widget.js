...
cpdefine("inline:com-chilipeppr-widget-autolevel", ["chilipeppr_ready", "ThreeHelvetiker", "Three"], function () {
    ...
    return {
        ...
        modalComplete: function() {
            ...
            // Find the Z work offset.  Probing operates using machine coordinates,
            // but our offsets need to be in work coordinates.
	    this.status("Finding work offset.");
	    var that = this;
	    this.findWorkOffset(function() {
		    that.doNextStep();
	    });
            
        },
	workOffsetZ: 0,
	findWorkOffset: function(cb) {
		var that = this;
		var machinePos = null;
		var workPos = null;
		var handler = function(data) {
			...
			var gotResult = false;
			if ('pos' in json || 'mpo' in json) {
				json = { r: json };
			}
	                if ('r' in json && 'pos' in json.r) {
				workPos = [ json.r.pos.x, json.r.pos.y, json.r.pos.z ];
				gotResult = true;
			}
			if ('r' in json && 'mpo' in json.r) {
				machinePos = [ json.r.mpo.x, json.r.mpo.y, json.r.mpo.z ];
				gotResult = true;
			}
			if (gotResult && !machinePos) {
	    			that.send('{"mpo":null}\n');
			} else if (gotResult && !workPos) {
	    			that.send('{"pos":null}\n');
			} else if (gotResult) {
				// have both
				chilipeppr.unsubscribe("/com-chilipeppr-widget-serialport/recvline", that, handler);
				that.isSubToRecvline = false;
				that.workOffsetZ = machinePos[2] - workPos[2];
				cb();
			}
		};
		
		if (!that.isSubToRecvline) {
			chilipeppr.subscribe("/com-chilipeppr-widget-serialport/recvline", that, handler);
			that.isSubToRecvline = true;
		}
	    	that.send('{"pos":null}\n');
	},
        ...
        doNextStep: function() {
            ...
            */
            ...
            setTimeout(function() {
                ...
                //}
                var maxNegZ = parseFloat($('.autolevel-elem.al-coord.maxneg-z').val()) + that.workOffsetZ;
                var probeFeedRate = $('.autolevel-elem.al-coord.probe-fr').val();
        ...
        watchZ: function(probe, donecallback) {
            ...
            var callback = function(data) {
                ...
                if ('r' in json && 'prb' in json.r) {
                    ...
                    probe.z = json.r.prb.z - that.workOffsetZ;
